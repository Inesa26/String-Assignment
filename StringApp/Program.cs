...
Console.WriteLine($"The word count of this string is: {count}");
Console.ReadLine();

//- Display the sentence count of this string
string[] sentenceArray = text.Split('.');
count = 0;
foreach (string element in sentenceArray)
{
    count++;
}
Console.WriteLine($"The sentence count of this string is: {count}");
Console.ReadLine();
...
foreach (string element in wordArray)
{
    if (element.Equals("encapsulation", StringComparison.OrdinalIgnoreCase))
        count++;
}
Console.WriteLine($"The word \"encapsulation\" appears in this string: {count} times");
Console.ReadLine();
...
for (int i = wordArray.Length - 1; i >= 0; i--) {
    reverseArray.Append(wordArray[i] + " ");
}
